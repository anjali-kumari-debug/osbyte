...
process_t* execute(const char* prog_name, int argc, const char* argv[]){
    ...
    if (list_directory_contents("/boot/osbyte/bin/", listptr, listsize, 0)){
        ...
        for (uint32_t i = 0; i < list->count; i++){
            ...
            if (*f){
                if (strcmp_case(f, full_name,true) == 0){
                    string path = string_format("/boot/osbyte/bin/%s",full_name);
                    file fd = {};
...
system_module bin_module = (system_module){
    ...
    .swrite = 0,
    .readdir = 0,
};//TODO: symlinks to link /bin to /boot/osbyte/bin
